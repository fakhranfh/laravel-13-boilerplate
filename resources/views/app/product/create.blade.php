...
<div class="py-12">
    <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
        ...
        <div class="bg-white dark:bg-gray-800 shadow-md rounded-lg p-6">
            <form method="POST" action="{{ route('product.store') }}" class="space-y-4" id="createForm" onsubmit="handleFormSubmit(event)">
                @csrf
        ...
        <div class="mb-4">
          ...
          <select id="status" name="status" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 dark:bg-gray-700 dark:border-gray-600 dark:text-white" required @if(isset($isView)) disabled @endif>
            ...
            <option value="success" {{ old('status', isset($item) ? $item->status : '') === 'success' ? 'selected' : '' }}>@lang('Success')</option>
            <option value="pending" {{ old('status', isset($item) ? $item->status : '') === 'pending' ? 'selected' : '' }}>@lang('Pending')</option>
            <option value="failed" {{ old('status', isset($item) ? $item->status : '') === 'failed' ? 'selected' : '' }}>@lang('Failed')</option>
          </select>
          @error('status')
            <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
          @enderror
